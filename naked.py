...
# NASA apk key and url
nasa_api_key = "u3vVSHRHyfRtSSpu8udeWPDOczoabyCVJybSAxwc"
...
if r.status_code == 200:
	...
	ast_safe = []
	ast_hazardous = []
#Print todays asteroid count
	if 'element_count' in json_data:
		ast_count = int(json_data['element_count'])
		logger.info("Asteroid count today: " + str(ast_count))
# If and else to now how many asteroids are near earth, with asteroids name and date
		if ast_count > 0:
...
#Close approaching asteroids near earth and all data for near asteroids from earth
					if len(val['close_approach_data']) > 0:
						if 'epoch_date_close_approach' and 'relative_velocity' and 'miss_distance' in val['close_approach_data'][0]:
							tmp_ast_close_appr_ts = int(val['close_approach_data'][0]['epoch_date_close_approach']/1000)
							tmp_ast_close_appr_dt_utc = datetime.utcfromtimestamp(tmp_ast_close_appr_ts).strftime('%Y-%m-%d %H:%M:%S')
							tmp_ast_close_appr_dt = datetime.fromtimestamp(tmp_ast_close_appr_ts).strftime('%Y-%m-%d %H:%M:%S')
#How fast is asteroid approaching 
							if 'kilometers_per_hour' in val['close_approach_data'][0]['relative_velocity']:
								tmp_ast_speed = int(float(val['close_approach_data'][0]['relative_velocity']['kilometers_per_hour']))
							else:
								tmp_ast_speed = -1
#How far from earth is assteroid
						...
						tmp_ast_speed = -1
						tmp_ast_miss_dist = -1
#Prints out data about asteroid
					logger.info("------------------------------------------------------- >>")
...
#Prints message, that no asteroids ar going to hit earth today :D
		else:
